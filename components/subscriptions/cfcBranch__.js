...
// Fetch CFC Packages
async function getcfcPackages() {
    const cfcPackagesQuery = 'SELECT package_id, name, price, validity_period FROM gym_packages WHERE branch_name = $1 ORDER BY package_id';
    const cfcPackagesResult = await pool.query(cfcPackagesQuery, ['CFC']);
    return cfcPackagesResult.rows;
}

// Display the subscription form for CFC branch
router.get('/', authenticate, checkRole(['superadmin' , 'admin']), async (req, res) => {
    try {
        const cfcPackages = await getcfcPackages();
        const loggedInUser = req.session.user;

        // Pass validity period to the view
        const validityPeriod = cfcPackages[0].validity_period; // Assuming the first package is selected
        ...
        res.render('subscriptions/cfcBranch', { branchName: 'CFC', cfcPackages, endDate, validityPeriod, endDateFormatted, loggedInUser });
    } catch (error) {
        console.error('Error fetching CFC packages:', error);
...
router.post('/', async (req, res) => {
    try {
        ...
        // Fetch CFC packages from the database
        const cfcPackages = await getcfcPackages();
        ...
        if (sessionCountResult.rows.length > 0) {
            ...
            if (packageValidityResult.rows.length > 0) {
                ...
                res.render('subscriptions/cfcBranch', { branchName: 'CFC', cfcPackages, endDate, validityPeriod, successMessage, loggedInUser });
            } else {
        ...
        console.error('Error handling CFC subscription form submission:', error);
        const errorMessage = 'An error occurred while assigning the package.';
        res.render('yourPage', { errorMessage, user });
    }
});
